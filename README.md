...
- **Unified Batch Execution**: Add multiple source directories at once. HemaFrag automatically builds nested pipeline jobs and isolates tracking parameters.
...
You can download the latest pre-compiled executables from the project's GitHub **Releases** page.

1. **macOS**: Download the `HemaFrag.app` bundle and double-click to run. (You may need to bypass Apple's unsigned app warning by right-clicking it and selecting `Open`).
2. **Windows**: Download `HemaFrag.exe` and execute it directly.
3. **Linux**: Download the ELF executable and run it via terminal (`./HemaFrag`).
...
```
*Wait for completion. You will find `HemaFrag.app` located in the `/dist` directory.*
...
```
*Wait for completion. You will find `HemaFrag.exe` exported to your local `/dist` directory.*
...
```
*Wait for completion. You will find the `HemaFrag` Linux executable exported to your local `/dist` directory.*
...
HemaFrag Diagnostics includes and builds on MIT-licensed upstream components from

...
class FsaFile:
    def __init__(
        ...
        # 2) Hvis ikke: bruk vår OUS-default for kjente laddere.
        # 3) Hvis fortsatt None: auto-detect med find_size_standard_channel().
        if size_standard_channel is not None:
            # Manuell override fra CLI / kall:
            self.size_standard_channel = size_standard_channel
        else:
            # --- OUS override: default-kanal for våre laddere ---
            if ladder in ("ROX400HD", "ROX500", "ROX", "LIZ500_250", "LIZ"):
                self.size_standard_channel = "DATA4"
            else:
        ...
        # Bare auto-detect hvis vi fortsatt ikke har en kanal:
        if self.size_standard_channel is None:
...
def cli():
    ...
    # NYTT: eksplisitt size-standard channel
    parser.add_argument(
        "--size-standard-channel",
        "--size_standard_channel",
        dest="size_standard_channel",
        required=False,
        default=None,
        type=str,
        help="Force a specific size-standard channel (e.g. DATA4 for ROX400HD).",
    )
    ...
    main(
        ...
        size_standard_channel=args.size_standard_channel,  # ← denne
...
def parse_fsa(
    ...
    try:
        fsa = FsaFile(
            ...
            size_standard_channel=size_standard_channel,  # ← viktig
        )
        return fsa
    except Exception as e:
        print_fail(f"Error parsing {fsa}: {e}")
        return None
